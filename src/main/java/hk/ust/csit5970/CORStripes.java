...
public class CORStripes extends Configured implements Tool {
	...
	private static class CORMapper1 extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			while (doc_tokenizer.hasMoreElements()) {
				TEXT.set(doc_tokenizer.nextToken());
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			while (!sorted_word_set.isEmpty()) {
				...
				while (iter.hasNext()) {
					...
					context.write(TEXT, STRIPE);
					STRIPE.clear();
				}


				sorted_word_set.remove(TEXT.toString());
			}
		}
	}
	...
	public static class CORStripesCombiner2 extends Reducer<Text, HashMapStringIntWritable, Text, HashMapStringIntWritable> {
		...
		protected void reduce(Text key, Iterable<HashMapStringIntWritable> values, Context context) throws IOException, InterruptedException {
			...
			while (iter.hasNext()) {
				for (String nextWord : iter.next().keySet()) {
					SUM_STRIPES.increment(nextWord);
				}
			}
	...
	public static class CORStripesReducer2 extends Reducer<Text, HashMapStringIntWritable, PairOfStrings, DoubleWritable> {
		...
		protected void reduce(Text key, Iterable<HashMapStringIntWritable> values, Context context) throws IOException, InterruptedException {
			...
			for (Entry<String, Integer>mapEntry : SUM_STRIPES.entrySet()) {
				String nextWord = (String)mapEntry.getKey();
				int value = word_total_map.get(word);
				int valueNext = word_total_map.get(nextWord);
				double sum = (double)SUM_STRIPES.get(nextWord);
				BIGRAM.set(word, nextWord);
				FREQ.set(sum/(value * valueNext));
